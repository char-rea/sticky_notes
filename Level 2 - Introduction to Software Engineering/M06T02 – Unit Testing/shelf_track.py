...
class TestShelfTrack(unittest.TestCase):
    ...
    def test_add_book(self):
        with sqlite3.connect(DATABASE_NAME) as db:
            ...
            cursor.execute("""
                ...
                VALUES (9001, 'Unit Test Book', 1290, 10)
            """)

            cursor.execute(
                "SELECT title FROM book WHERE id = 9001"
            )

            result = cursor.fetchone()

        self.assertEqual(result[0], "Unit Test Book")
    ...
    def test_update_quantity(self):
        with sqlite3.connect(DATABASE_NAME) as db:
            ...
            cursor.execute(
                "SELECT qty FROM book WHERE id = 9002"
            )
    ...
    def test_delete_book(self):
        with sqlite3.connect(DATABASE_NAME) as db:
            ...
            cursor.execute("""
                INSERT OR REPLACE INTO book
                VALUES (9003, 'Delete Test', 1290, 5)
            """)

            cursor.execute(
                "DELETE FROM book WHERE id = 9003"
            )

            cursor.execute(
                "SELECT * FROM book WHERE id = 9003"
            )
